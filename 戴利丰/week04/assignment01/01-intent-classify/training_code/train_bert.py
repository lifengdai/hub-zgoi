import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 指定使用第0号GPU（通常是独立显卡）
import pandas as pd
import torch
...
if torch.cuda.is_available():
    # 创建一个device对象，指定使用第0号GPU
    device = torch.device("cuda:0")
    # 或者简单地指定为"cuda"，默认使用0号GPU
...
labels = lbl.fit_transform(dataset_df[0].sample(n = 500, random_state = 20))
# 提取前500个文本内容
texts = list(dataset_df[1].sample(n = 500, random_state = 20))
